...
export function DatabaseSetup({ onConnected }: { onConnected: (needsBootstrap: boolean) => void }) {
  const [form, setForm] = useState({ server: "tcp:PUBLIC_IP,1433", database: "erp", user: "sqlserver", password: "", encrypt: true, trustServerCertificate: true });
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      onConnected(Boolean(body.needsBootstrap));
    } catch (error) { setMessage(error instanceof Error ? error.message : "Koneksi SQL Server gagal."); }
    finally { setLoading(false); }
  }
  return <div className="setup-page"><div className="auth-brand"><span>E</span> ERPJIN</div><form className="setup-card" onSubmit={submit}>
    ...
    <small className="setup-note">Setelah terhubung, ERPJIN menyiapkan schema lalu membuka pembuatan akun Administrator pada database ini.</small>
  </form></div>;
}
