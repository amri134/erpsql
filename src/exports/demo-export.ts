...
type ExportBranding = { appName: string; companyName: string };
const defaultBranding: ExportBranding = { appName: "ERPJIN", companyName: "PT Hajijin Amri" };
...
export async function createDemoExcel(moduleCode: string, rows: DemoRow[], branding: ExportBranding = defaultBranding): Promise<Buffer> {
  const workbook = new ExcelJS.Workbook();
  workbook.creator = `${branding.appName} - ${branding.companyName}`;
  ...
  const title = sheet.getCell(1,1); title.value = `${branding.appName} - ${moduleNames[moduleCode] ?? moduleCode}`; title.font = { bold:true,size:18,color:{argb:"FFFFFFFF"} }; title.fill={type:"pattern",pattern:"solid",fgColor:{argb:"FF17315C"}}; title.alignment={vertical:"middle"};
  sheet.getRow(1).height=34;
  sheet.mergeCells(2,1,2,Math.max(keys.length,1)); sheet.getCell(2,1).value=`${branding.companyName} | Data demo dari SQL | ${new Date().toLocaleString("id-ID")}`; sheet.getCell(2,1).font={italic:true,color:{argb:"FF637087"}};
...
export async function createDemoPdf(moduleCode: string, rows: DemoRow[], branding: ExportBranding = defaultBranding): Promise<Buffer> {
  ...
  return new Promise((resolve,reject)=>{
    ...
    doc.rect(0,0,842,72).fill("#17315c"); doc.fillColor("#ffffff").font("Helvetica-Bold").fontSize(20).text(`${branding.appName} - ${moduleNames[moduleCode] ?? moduleCode}`,36,24);
    doc.fillColor("#65748b").font("Helvetica").fontSize(9).text(`${branding.companyName} | Data demo dari SQL | Dicetak ${new Date().toLocaleString("id-ID")}`,36,86);
